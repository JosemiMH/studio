...
type PortfolioTranslations = AllTranslations['es']['portfolio'];
type Category = 'all' | 'luxury' | 'urban' | 'resorts' | 'wellness';

const PortfolioSection = ({ translations }: { translations: PortfolioTranslations }) => {
  const [filter, setFilter] = useState<Category>('all');

  const filteredProjects =
    filter === 'all'
      ? translations.projects
      : translations.projects.filter((p: any) => p.category === filter);
            ...
            return (
              <Card key={project.id} className="overflow-hidden group transition-all duration-300 hover:shadow-2xl hover:-translate-y-2">
                ...
                <CardContent className="p-6">
                  <p className="text-sm font-semibold text-primary">{translations.categories[project.category]}</p>
